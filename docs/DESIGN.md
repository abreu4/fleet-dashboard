...
| **Apollo** | Mission Control, 1969, and the room's personality is procedural calm: grey-green console bays, every status dot a square indicator lamp, one lit word for everything, GET and FLIGHT's answer at the foot of the room. Collapse the board for the front wall and the poll: the plotboard tracks every working session by how long since it last wrote (a note or a prompt counts) — 2:30 of silence and it is behind the Moon, LOS, until it writes again; under it the Flight Director's poll, one station per real figure (BOOSTER cpu, EECOM pressure or battery, GUIDO the agents' memory, SURGEON blocked, CAPCOM waiting on you, INCO who can be heard, NETWORK this board's own snapshot age). The poll is a ritual: when the band comes up and whenever FLIGHT's answer would change, FLIGHT calls the room for the next event on the flight plan (Apollo 11's, at a twentieth of the pace — a working day is a mission), the lamps go dark and each station answers in turn, FLIGHT gives the verdict, MASTER ALARM while it is NO GO. Between polls the loop speaks the way the room did — every line opens with who is called and who is calling — and after two quiet minutes the PAO reads the room to the public. Sounds are the Quindar tones |
